...
(async () => {
  ...
  const initializeWithToken = async (token, usertoken) => {
    ...
    const clearWebhooks = async () => {
      ...
      const resultGetWebhooks = await request(`${twitchApiUrl}${helixEventsubSubscriptionPath}`, {
        method: 'GET',
      ...
      for (const webhook of resultGetWebhooks.data) {
        ...
        await request(`${twitchApiUrl}${helixEventsubSubscriptionPath}?id=${webhookId}`, {
          method: 'DELETE',
          headers: twitchApiAuthHeaders,
        });
    ...
    const createWebhook = (broadcasterId, type) => request(`${twitchApiUrl}${helixEventsubSubscriptionPath}`, {
      method: 'POST',
      headers: twitchApiAuthHeaders,
    }, {
      'type': type,
    ...
    // Get broadcaster id
    const resultGetBroadcasterId = await request(`${twitchApiUrl}${helixUsersPath}?login=${clientLogin}`, {
      method: 'GET',
      headers: twitchApiAuthHeaders,
    });
  ...
  app.get(`/${endpointLogin}`, (req, res) => {
    ...
    res.redirect(url.make(`${twitchAuthUrl}/authorize`, params));
  });
  ...
  app.listen(port, async () => {
    ...
    const resultGetToken = await request(url.make(`${twitchAuthUrl}/token`, params), {
      method: 'POST',
    });
