...
const doRequest = (params, body) => new Promise(async (resolve) => {
  let responseData = '';
...
(async () => {
  ...
  const initializeWithToken = async (token, usertoken) => {
    ...
    const resultGetWebhooks = await doRequest({
      host: twitchApi,
    ...
    const createWebhook = (broadcasterId, type) => doRequest({
      host: twitchApi,
      path: helixEventsubSubscriptionPath,
      method: 'POST',
      headers: twitchApiAuthHeaders,
    ...
    // Get broadcaster id
    const resultGetBroadcasterId = await doRequest({
      host: twitchApi,
  ...
  app.listen(port, async () => {
    ...
    const resultGetToken = await doRequest({
      host: 'id.twitch.tv',
