...
module.exports = (c) => {
  ...
  return {
    ...
    create: (broadcasterId, type) => lib.tools.request(lib.config.twitch.url.api.helixEventsub, {
      method: 'POST',
      headers: getTwitchApiAuthHeaders(),
    }, {
      'type': type,
    ...
    createAll: async (broadcasterId, callback) => {
      for (const type of lib.data.types) {
        await this.create(broadcasterId, type, callback);
      }
    },
  };
};
