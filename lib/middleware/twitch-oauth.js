...
module.exports = (c) => {
  ...
  c.app.get('/oauthcallback', (req, res) => {
    res.send(`<script>document.location = '${c.tunnelUrl}/${endpointLogin}' + document.location.hash.replace('#', '?')</script>`);
  });
  ...
  c.app.get(`/${endpointLogin}`, async (req, res) => {
    ...
    const params = {
      ...
      redirect_uri: `${c.tunnelUrl}/oauthcallback`,
      response_type: 'token',
      scope: scope,
      state: oauthState,
    };
  ...
  return async () => {
    ...
    try {
      ...
      const loginUrl = `${c.tunnelUrl}/${endpointLogin}`;
